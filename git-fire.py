...
current_branch = subprocess.run("git symbolic-ref HEAD", capture_output=True).stdout.decode('utf-8').replace("\n", "").replace("\r", "")
repo_base = subprocess.run("git rev-parse --show-toplevel", capture_output=True).stdout.decode('utf-8').replace("\n", "").replace("\r", "")
user_email = subprocess.run("git config user.email", capture_output=True).stdout.decode('utf-8').replace("\n", "").replace("\r", "")
